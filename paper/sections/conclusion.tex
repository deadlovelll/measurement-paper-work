...
eight threads --- confirms that the lock rather than the build is responsible.

\textbf{Where the mechanisms are not the ones usually named.} The largest levers in the compute
...
\textbf{The costs, which are the part usually missing.} Four numbers that decide a choice and
are rarely reported: the point at which a JIT pays for itself, which moves from 7 calls to 7335
...
($118.5$ against $\meas{med:b2_branchy-314:tokenize/c_ctypes:ms}{22.3}$\,ms) while the calls that pass only integers are within $\meas{pct:b2_branchy-314:binarytrees/rust_ctypes|b2_branchy-pypy311:binarytrees/rust_ctypes}{7}\%$, so
``switch runtime'' and ``move the hot loop to C'' can exclude each other; and CPython's own
experimental JIT is a $\meas{geomean:b5_buildcfg-plain|b5_buildcfg-jit}{1.04}\times$ proposition overall that is not free either --- it improves 10
...
and not by which tool is fashionable, because within a workload class the spread between tools
