...
The easiest way to organise advice in this area is by technology --- ``use Numba for this, use C for
that''. The measurements put the workload first and the technology second, for reasons
...
\paragraph{3. Every gain has a cost, and here it is measured alongside it.}
...
single-thread slowdown (\S\ref{sec:rq5}). Static Python costs a rewrite in primitive types
...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.22\linewidth}p{0.25\linewidth}p{0.17\linewidth}p{0.26\linewidth}}
    ...
    irregular graph traversal
      ...
      & $\meas{ratio:b2_branchy-314:bfs/cpython_loop|b2_branchy-314:bfs/cython}{9.9}\times$ (Cython and C within half a per cent)
