...
def generate_deopt_table(analysis: Analysis, out: CWriter) -> None:
    out.emit("extern uint8_t _PyOpcode_Deopt[256];\n")
    out.emit("#ifdef NEED_OPCODE_METADATA\n")
    out.emit("uint8_t _PyOpcode_Deopt[256] = {\n")
...
def generate_cache_table(analysis: Analysis, out: CWriter) -> None:
    out.emit("extern uint8_t _PyOpcode_Caches[256];\n")
    out.emit("#ifdef NEED_OPCODE_METADATA\n")
    out.emit("uint8_t _PyOpcode_Caches[256] = {\n")
    for inst in analysis.instructions.values():
