...
void Ci_InitOpcodes() {
    ...
    // Ensure CinderX's private opcode-metadata tables (_CiOpcode_Caches/_Deopt, the
    // copies referenced by CinderX's own eval loop) are populated. These are the
    // macro-renamed instances from cinder_opcode.h; they are writable thanks to
    // CinderX research patch #1 (de-const of _PyOpcode_Caches/_Deopt in the header).
    for (size_t i = 0; i < sizeof(_CiOpcode_Caches) / sizeof(_CiOpcode_Caches[0]); i++) {
        _PyOpcode_Caches[i] = _CiOpcode_Caches[i];
    }
    for (size_t i = 0; i < sizeof(_CiOpcode_Deopt) / sizeof(_CiOpcode_Deopt[0]); i++) {
